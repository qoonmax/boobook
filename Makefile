...
create_migration:
	GOOSE_DRIVER=$(DATABASE_DRIVER) GOOSE_DBSTRING=$(WRITE_DATABASE_DBSTRING) GOOSE_MIGRATION_DIR=$(DATABASE_MIGRATION_DIR) goose create $(name) sql

migrate_up:
	GOOSE_DRIVER=$(DATABASE_DRIVER) GOOSE_DBSTRING=$(WRITE_DATABASE_DBSTRING) GOOSE_MIGRATION_DIR=$(DATABASE_MIGRATION_DIR) goose up
